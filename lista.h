...
private:
	...
	public:
		...
		Nodo(Nodo *ant, const T &elem, const int &peso, Nodo *sig) : 
		    _elem(elem), _sig(sig), _ant(ant), _peso(peso) {}
	...
	*/
	static Nodo *insertaElem(const T &e, const int &p, Nodo *nodo1, Nodo *nodo2) {
		Nodo *nuevo = new Nodo(nodo1, e, p, nodo2);
...
public:
	...
	*/
	void pon_ppio (const T &elem, const int &peso) {
		_prim = insertaElem(elem, peso, NULL, _prim);
		_numElems ++;
	...
	*/
	void pon_final(const T &elem, const int &peso) {
		_ult = insertaElem(elem, peso, _ult, NULL);
		_numElems ++;
		if (_prim == NULL) 
			_prim = _ult;
	}
	...
	*/
	...
	const T &peso(unsigned int idx) const {
		if (idx >= _numElems)
			throw EAccesoInvalido(); 
		Nodo *aux = _prim;
		for (unsigned int i = 0; i < idx; ++i)
